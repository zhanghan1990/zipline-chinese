...
from zipline.finance.commission import PerShare, PerTrade, PerDollar,OrderCost
...
class TradingAlgorithm(object):
    ...
    *args, **kwargs
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.instant_fill = kwargs.pop('instant_fill', True)
...
""".strip().format(class_name=self.__class__.__name__,
    ...
    def order(self, sid, amount,
        ...
        strsid=sid
        if type(sid) != type(1):
            sid=self.asset_finder.retrieve_asset(sid)
        ...
        #股票数目检测
        if self.portfolio.positions[sid].amount+ amount < 0 and self.long==True:
            logging.warning("%s:原始下单数目为%d:您当前可平仓的证券数量不足,调整为%d,%s"%(self.datetime,amount,self.portfolio.positions[sid].amount,self.portfolio.positions[sid]))
            amount=-1*self.portfolio.positions[sid].amount
        
        #本金是否满足要求
        last_price = self.trading_client.current_data[sid].price 
        if self.portfolio.cash < last_price*amount and amount > 0:
            original=amount
            amount =self.portfolio.cash/last_price
            n=amount%100
            amount=amount-n
            logging.warning("%s:原始下单数目为%d:您当前现金数量不足,调整为%d,%s"%(self.datetime,original,amount,self.portfolio.positions[sid]))
        ...
        #check whether the stock stop limit or not
        ...
        return self.blotter.order(sid, amount, style,self.commission)
    ...
    def set_commission(self, commission):
        if not isinstance(commission, (PerShare, PerTrade, PerDollar,OrderCost)):
