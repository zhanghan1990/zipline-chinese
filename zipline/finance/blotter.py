...
from zipline.finance.commission import PerShare,OrderCost
...
class Blotter(object):
    ...
    def __init__(self):
        self.transact = transact_partial(VolumeShareSlippage(), OrderCost())
...
""".strip().format(class_name=self.__class__.__name__,
    ...
    def order(self, sid, amount, style, commission,order_id=None):
    ...
    def process_trade(self, trade_event):
        ...
        if trade_event.change >=0.099 and trade_event.open==trade_event.close:
            orders=self.open_orders[trade_event.sid]
            for o in orders:
                print(u"%s:开盘%s 涨停，买入失败"%(trade_event.dt,trade_event.code))
                return 

        #for the increasement limit
